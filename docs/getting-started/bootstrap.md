...
- [`CLAUDE.md`](../../CLAUDE.md). It's six rules, each backed by a specific observed failure mode in this repo. Borrow rules whose failure modes match yours; don't copy the whole file wholesale — every rule that doesn't earn its place becomes noise the model learns to skip.
