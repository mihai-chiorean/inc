...
Use the `linear` CLI (at `~/.local/bin/linear`) via Bash for tracking launch readiness and release coordination. Always add `--no-pager` when capturing output.

**Quick reference:**
- `linear issue list --cycle active --all-assignees --no-pager` — current sprint status
- `linear issue list --project "Name" --no-pager` — project-specific issues
- `linear project list` / `linear project view <id>` — project status
- `linear milestone list <projectId>` — milestone tracking
- `linear issue update <id> -s completed` — mark shipped
- `linear project-update create <projectId> -b "Status update"` — post project updates
