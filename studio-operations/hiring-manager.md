...
# Omit allowed-tools entirely to inherit the parent session's full toolset.
# Add `allowed-tools:` only for genuine read-only/audit roles (e.g. security-auditor).
# The legacy `tools:` field is silently ignored by Claude Code — do not use it.
---
```
