...
def parse_input(args: argparse.Namespace) -> tuple[list[str], dict | None]:
    ...
    if args.from_suggest:
        ...
        meta = {
            "hr_commit": payload.get("hr_commit"),
...
def apply_agent(paths: Paths, agent_id: str, manifest: dict, hr_commit: str,
                manifest_entry: dict) -> dict:
    """Process one agent. Returns the lockfile entry for it."""
...
def write_lockfile(paths: Paths, hr_commit: str, staffed: dict) -> None:
    ...
    paths.lock_path.write_text(out, encoding="utf-8")
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true", help="proceed despite HR drift or dirty HR repo")
    ...
    if suggest_meta is not None:
        try:
            ...
            print(f"error: {exc}", file=sys.stderr)
            return 2
        if suggest_meta.get("hr_commit") and suggest_meta["hr_commit"] != current_head and not args.force:
            print(
        ...
        if suggest_meta.get("manifest_hash") and suggest_meta["manifest_hash"] != current_mh and not args.force:
            print(
    ...
    for canonical, entry in resolved:
        try:
            staffed[canonical] = apply_agent(paths, canonical, manifest, hr_commit, entry)
        except ValueError as exc:
    ...
    if failures:
        ...
        print(f"\n{len(failures)} agent(s) failed to apply. Lockfile NOT written.", file=sys.stderr)
        return 5
