...
### Container runtime invariant: containerd only, no docker on device

WendyOS devices run **containerd** as the sole container runtime (via `wendy-agent`). Docker is **not** installed on devices and should not be installed on them — on disk-constrained edge hardware, running two runtimes side by side silently accumulates image layers, volumes, and buildkit caches on `/` until the device wedges. This has been observed in production: 8+ GB of dangling docker state on a 22 GB rootfs caused journald to stop writing and the USB gadget driver to hang.

- To inspect what's running on a device: `ctr -n default tasks ls` / `ctr -n default containers ls` / `ctr -n default images ls`.
- To run a third-party image that ships for `docker run`: use **`nerdctl`** (same OCI image format, containerd-backed). `nerdctl run ...` is a near drop-in.
- The dev-host `docker buildx` that `wendy run` uses is a *build-time* tool — the output (an OCI image) is pushed to the device's containerd registry; the docker daemon does not run on the device.
