---
name: cso
description: 'Chief Security Officer audit — OWASP Top 10 + STRIDE threat model + secrets archaeology + supply-chain + CI/CD review, producing a read-only Security Posture Report with severity, confidence, exploit scenario, and remediation. Use when: (1) user asks "audit this for security", "find vulnerabilities", "threat-model this", (2) reviewing auth/secrets/dependencies/CI before shipping, (3) "/cso" is invoked, (4) an idea has gotten concrete enough to have an attack surface. Read-only: produces findings, never changes code. Adapted from Garry Tan gstack cso (MIT), stripped of gstack telemetry/gbrain/learnings plumbing.'
---
...
**HARD GATE:** Read-only. Never modify code. The only output is the report.

## Arguments

- `/cso` — full daily audit (all phases, 8/10 confidence gate)
- `/cso --comprehensive` — deep scan (all phases, 2/10 bar — surfaces more, flags low-confidence as TENTATIVE)
...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use the Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
ls Package.swift 2>/dev/null && echo "STACK: Swift"
...
grep -q "hummingbird\|vapor" Package.swift 2>/dev/null && echo "FRAMEWORK: Swift server"
```
...
The HIGH-tier credential prefixes the archaeology greps target are the canonical set: `AKIA`, `ghp_`, `sk-ant-`, `sk_live_`, `xoxb-`, `-----BEGIN ... PRIVATE KEY-----`, etc.
...
[ -f Package.swift ] && echo "DETECTED: swiftpm"
```
...
Scan installed AI coding agent skills for malicious patterns. Published skills are a real supply-chain vector — a meaningful fraction carry security flaws and some are outright malicious.
...
**FP rules:** This repo's own skills (those resolving into the `inc` skills directory) are trusted. Skills that use `curl` for legitimate purposes (downloading tools, health checks) need context — only flag when the target URL is suspicious or when the command includes credential variables.
...
9. Memory safety issues in memory-safe languages (Rust, Go, Java, C#, Swift)
...
Recommend adding `.security-reports/` to `.gitignore` if it isn't already — security reports should stay local.
