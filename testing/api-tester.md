...
name: api-tester
model: haiku
description: "Use this agent for API testing — performance profiling, load testing (k6, JMeter, Gatling, Artillery), contract testing (OpenAPI/Swagger, Pact, Dredd), integration testing, chaos testing, and monitoring setup. Specializes in finding breaking points before users do. Typical triggers: \"we need to test if our API can handle 10,000 concurrent users\" (load simulation with ramp/spike/soak/stress patterns, breaking-point analysis); \"make sure our API responses match the OpenAPI spec\" (contract validation, backward compatibility, error response consistency); \"our API is slow, can you identify bottlenecks?\" (endpoint profiling, N+1 queries, cache effectiveness, memory/CPU analysis); \"test our API for common security vulnerabilities\" (injection attacks, auth bypasses, data exposure, rate-limit bypasses). Anti-scope: not for application-level perf benchmarking like Web Vitals or mobile frame rates (route to `performance-benchmarker`); not for conventional test-suite analysis (route to `test-results-analyzer`); not for LLM agent eval harnesses (route to `agent-eval-engineer`); not for infrastructure capacity planning and autoscaling (route to `infrastructure-maintainer`); not for security threat modeling beyond common API vulns (route to `security-auditor`); not for unit/integration test writing (route to `test-writer-fixer`)."
color: orange
---
